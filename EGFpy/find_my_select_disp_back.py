...
from find_associated_EGF_and_disp import (file2list,
    ...
    find_sta_pair)
from GVImageGroupWidget import GVImageGroupWidget
from InterStaDist import InterStaDist
from InterStaDistWidget import StationMapWidget
...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, configfile):
        ...
        self.setWindowTitle('Find it back, you stupid! (Version 1.0)')
    ...
    def setupUI(self):
        ...
        self.countlabel = QtGui.QLabel('Dispersions left: {}/{}'.format(\
                    self.__get_select_disp_count(), self.__get_disp_count('A')))
    ...
    def start(self):
        ...
        self.sta_pair_selected = find_sta_pair(sta_group1, sta_group2,
                                          self.sta_pair_list)
        self.sta_pair_selected = self.__filter_out_selected_sta_pair()
    ...
    def __get_select_disp_count(self):
        return len(os.listdir(self.accepted_disp_folder))
    ...
    def onclicknextbutton(self):
        ...
        if index == len(self.sta_pair_list) - 1:
            ...
            for sta_pair in self.sta_pair_list[index+1:]:
                if self.disp_checked_dict[sta_pair] == 'A' and \
                   self.__no_select_disp_file(sta_pair):
                    break
            #sta_pair = self.sta_pair_list[index+1]

            self.update(sta_pair)

    def __filter_out_selected_sta_pair(self):
        sta_pair_list = []
        for sta_pair in self.sta_pair_selected:
            if self.__no_select_disp_file(sta_pair) and \
               self.disp_checked_dict[sta_pair] == 'A':
                sta_pair_list.append(sta_pair)
        return sta_pair_list

    def __no_select_disp_file(self, sta_pair):
        disp_filename = os.path.basename(\
            self.disp_dict[sta_pair])
        modified_disp_file = os.path.join(\
            self.accepted_disp_folder, disp_filename)
